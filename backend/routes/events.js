...
//     // Remove all attendees from the event first
...
//     const deleteResult = await events.deleteOne({ _id: req.params.id });
...
router.delete("/:id", authMiddleWare, async (req, res, next) => {
  try {
    ...
    // Check if any services are selected for the event
    if (event.services.length > 0) {
      return res.status(400).send("Cannot delete event with selected services");
    }
