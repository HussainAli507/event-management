...
router.post('/login', async (req, res) => {
  ...
  try {
    const user = await users.findOne({ $or: [{ username: username }, { org: org }] });
    if (!user) {
      return res.status(400).json({ message: 'Invalid username' });
    }

    const isMatch = await bcrypt.compare(password, user.password);
    //bcrypt.hash("user123",10).then(hash=>console.log(hash))
    //bcrypt.hash("user000",10).then(hash=>console.log(hash))
    if (!isMatch) {
