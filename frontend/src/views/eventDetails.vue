...
// customer validation for the date
const validDate = (value) => {
  const date = new Date(value);
  return !isNaN(date);
};

// setting up vuelidate validations
...
// fetch the event data when component is mounted
onMounted(async () => {
  try {
    ...
    event.value = {
      ...eventResponse,
      date: new Date(eventResponse.date).toISOString().substring(0, 10),
    };
...
const submitDeleteEvent = async () => {
  if (event.value.attendees.length > 0) {
    toast.infor(
      "Event cannot be deleted because it has one or more attendees."
    );
    return;
  }
  try {
    await deleteEvent(route.paramas.id);
    toast.sucess("Event deleted successfully!");
    router.push("/findevents");
  } catch (error) {
